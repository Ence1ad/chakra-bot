...
from cache.trackers_redis_manager import is_redis_hexists_tracker, \
    redis_hget_tracker_data
from config import settings
from db.operations.actions_operations import select_category_actions, create_actions, \
    delete_action
...
from tgbot.handlers.actions_handlers.read_actions import _actions_list, \
    _get_action_operation
from tgbot.keyboards.app_buttons import AppButtons
from tgbot.keyboards.callback_factories import ActionOperation, CategoryCD, \
    CategoryOperation, ActionCD
...
class TestActionsHandlers:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (MAIN_USER_ID, ActionState.WAIT_CATEGORY_DATA, CategoryOperation.READ,
             'selected_category', does_not_raise()),
            (OTHER_USER_ID, ActionState.WAIT_CATEGORY_DATA, CategoryOperation.READ,
             'selected_category', does_not_raise()),
            (USER_WITHOUT_ACTION, ActionState.WAIT_CATEGORY_DATA,
             CategoryOperation.READ, 'empty_actions_text', does_not_raise()),
            (USER_WITHOUT_ACTION, ActionState.WAIT_CATEGORY_DATA,
             CategoryOperation.READ, 'selected_category',
             pytest.raises(FluentReferenceError)),
            (OTHER_USER_ID, ActionState.WAIT_CATEGORY_DATA, CategoryOperation.READ,
             'empty_actions_text', pytest.raises(AssertionError)),
            (OTHER_USER_ID, ActionState.WAIT_CATEGORY_DATA, ActionOperation.READ,
             'empty_actions_text', pytest.raises(AssertionError)),
            (OTHER_USER_ID, ActionState.GET_NAME, CategoryOperation.READ,
             'empty_actions_text', pytest.raises(AssertionError)),
        ]
    )
    async def test_actions_main_menu_handler(
            self, user_id: int, answer_text: str, expectation: Any,
            execute_callback_query_handler,
        ...
        with expectation:
            ...
            if actions:
                ...
                assert handler_returns.text == i18n.get(
                    answer_text, category_name=category_name, new_line='\n',
    ...
    @pytest.mark.parametrize(
        ...
        [
            (USER_WITHOUT_ACTION, AppButtons.actions_btn_source.USER_ACTIONS.name,
             'empty_actions_text', does_not_raise()),
            (OTHER_USER_ID, AppButtons.actions_btn_source.USER_ACTIONS.name,
             'show_action_text', does_not_raise()),
            (MAIN_USER_ID, AppButtons.actions_btn_source.USER_ACTIONS.name,
             'show_action_text', does_not_raise()),
            (MAIN_USER_ID, AppButtons.actions_btn_source.USER_ACTIONS.name,
             'empty_actions_text', pytest.raises(AssertionError)),
            (MAIN_USER_ID, AppButtons.actions_btn_source.USER_ACTIONS.name, '',
    ...
    async def test_display_actions(
            ...
            expectation: Any, dispatcher: Dispatcher,
    ...
    @pytest.mark.parametrize(
        "user_id, data, answer_text, expectation",
        [
            (MAIN_USER_ID, AppButtons.actions_btn_source.UPDATE_ACTIONS.name,
             'select_action_text', does_not_raise()),
            (MAIN_USER_ID, AppButtons.actions_btn_source.DELETE_ACTIONS.name,
             'select_action_text', does_not_raise()),
            (USER_WITHOUT_ACTION, AppButtons.actions_btn_source.UPDATE_ACTIONS.name,
             'empty_actions_text', does_not_raise()),
            (USER_WITHOUT_ACTION, AppButtons.actions_btn_source.DELETE_ACTIONS.name,
             'empty_actions_text', does_not_raise()),
        ]
    )
    async def test_collect_actions_btn_source_handler(
            self, user_id: int, answer_text: str, data: str,
            expectation: Any, execute_callback_query_handler, buttons: AppButtons, i18n,
            db_session_fixture, dispatcher, bot, chat_fixt_fact
    ) -> None:
        chat = await chat_fixt_fact(user_id)
        key = StorageKey(bot_id=bot.id, chat_id=chat.id, user_id=user_id)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (MAIN_USER_ID, ActionState.WAIT_CATEGORY_DATA,
             AppButtons.actions_btn_source.CREATE_ACTIONS.name,
             'action_limit_text', does_not_raise()),
            (USER_WITHOUT_ACTION, ActionState.WAIT_CATEGORY_DATA,
             AppButtons.actions_btn_source.CREATE_ACTIONS.name,
             'new_action_text', does_not_raise()),
            (OTHER_USER_ID, ActionState.WAIT_CATEGORY_DATA,
             AppButtons.actions_btn_source.CREATE_ACTIONS.name,
             'new_action_text', does_not_raise()),
            (MAIN_USER_ID, ActionState.WAIT_CATEGORY_DATA,
             AppButtons.actions_btn_source.CREATE_ACTIONS.name, None,
             pytest.raises(AssertionError)),
    ...
    async def test_prompt_name_4_new_action_handler(
            ...
            create_actions_more_than_limit, i18n: TranslatorRunner,
            buttons: AppButtons, state: FSMContext
    ) -> None:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (OTHER_USER_ID, ActionState.GET_NAME, 'my_new_action',
             'added_new_action_text', does_not_raise()),
            (OTHER_USER_ID, ActionState.GET_NAME, 'my_new_action', 'action_exists_text',
             does_not_raise()),
            (OTHER_USER_ID, ActionState.GET_NAME, 'my_new_action', 'action_exists_text',
             does_not_raise()),
            (OTHER_USER_ID, ActionState.GET_NAME, 'not_exist!', 'action_exists_text',
             pytest.raises(AssertionError)),
            (OTHER_USER_ID, ActionState.WAIT_CATEGORY_DATA, 'best_action',
             'added_new_action_text',
             pytest.raises(AssertionError)),
        ]
    )
    async def test_create_action_handler(
            self, user_id: int, answer_text: str, new_action_name: str,
            expectation: Any, buttons: AppButtons, execute_message_handler,
            i18n: TranslatorRunner, state: FSMContext, chat_fixt_fact
    ) -> None:
        chat: Chat = await chat_fixt_fact(user_id)
        test_message = Message(message_id=1234578, date=datetime.now(), chat=chat,
                               text=new_action_name)

        with expectation:
            handler_returns = await execute_message_handler(user_id,
            ...
            if not isinstance(new_action_name, str):
                ...
                assert handler_returns.text == i18n.get(
                    answer_text, new_action_valid_name=new_action_name
                )
                assert handler_returns.reply_markup == await menu_inline_kb(
                    await buttons.actions_btn_source.action_menu_buttons(), i18n)
    ...
    @pytest.mark.parametrize(
        "user_id, state, operation, answer_text, expectation",
        [
            (MAIN_USER_ID, ActionState.UPDATE_NAME, ActionOperation.UPD,
             'new_action_text', does_not_raise()),
            (OTHER_USER_ID, ActionState.UPDATE_NAME, ActionOperation.UPD,
             'new_action_text', does_not_raise()),
            (USER_WITHOUT_ACTION, ActionState.UPDATE_NAME, ActionOperation.UPD,
             'new_action_text', pytest.raises(AssertionError)),
        ]
    )
    async def test_prompt_new_action_name(
        ...
        category_id = state_data['category_id']
        category_name = state_data['category_name']
        actions_lst = await select_category_actions(user_id, category_id=category_id,
                                                    db_session=db_session_fixture)
        data = None
        if actions_lst:
        ...
        with expectation:
            assert actions_lst
            assert (await dispatcher.fsm.storage.get_data(key)).get(
    ...
    @pytest.mark.parametrize(
        "user_id, state, new_action_name, answer_text, expectation",
        [
            (OTHER_USER_ID, ActionState.UPDATE_NAME, 'upd_action_name',
             'upd_action_text', does_not_raise()),
            (OTHER_USER_ID, ActionState.UPDATE_NAME, 'upd_action_name',
             'action_exists_text', does_not_raise()),
            (MAIN_USER_ID, ActionState.UPDATE_NAME, 'best_act', 'upd_action_text',
             does_not_raise()),
            (USER_WITHOUT_ACTION, ActionState.UPDATE_NAME, 'upd_action_name',
             'valid_data_text', does_not_raise()),
        ]
    )
    async def test_upd_action_name(
            self, user_id: int, new_action_name: str, state: FSMContext,
            expectation: Any, execute_message_handler, i18n: TranslatorRunner,
            answer_text: str,  buttons: AppButtons, redis_cli, create_tracker_fixt_fact,
            chat_fixt_fact, bot, dispatcher
    ) -> None:
        if user_id == TestActionsHandlers.USER_WITHOUT_ACTION:
            chat: Chat = await chat_fixt_fact(user_id)
            key = StorageKey(bot_id=bot.id, chat_id=chat.id, user_id=user_id)
            category_id = 999
            action_id = 999
        ...
        if user_id == MAIN_USER_ID:
            await create_tracker_fixt_fact(user_id, category_id=1,
                                           category_name='new_cat', action_id='10',
                                           action_name=new_action_name,
                                           tracker_id='1')
        chat: Chat = await chat_fixt_fact(user_id)
        test_message = Message(message_id=123456, date=datetime.now(), chat=chat,
                               text=new_action_name)
        ...
        with expectation:
            ...
            if not isinstance(new_action_name, str):
                ...
                assert handler_returns.reply_markup == await menu_inline_kb(
                    await buttons.actions_btn_source.action_menu_buttons(), i18n)
                if user_id == MAIN_USER_ID:
                    assert await redis_hget_tracker_data(
                        user_id, redis_client=redis_cli, key='action_name'
                    ) == str(new_action_name).encode(encoding='utf-8')

    @pytest.mark.parametrize(
        "user_id, state, operation, answer_text, expectation",
        [
            (MAIN_USER_ID, ActionState.WAIT_CATEGORY_DATA, ActionOperation.DEL,
             'rm_action_text', does_not_raise()),
            (OTHER_USER_ID, ActionState.WAIT_CATEGORY_DATA, ActionOperation.DEL,
             'rm_action_text', does_not_raise()),
            (OTHER_USER_ID, ActionState.WAIT_CATEGORY_DATA, ActionOperation.DEL,
             'action_not_exists_text',
             does_not_raise()),
            (MAIN_USER_ID, ActionState.WAIT_CATEGORY_DATA, ActionOperation.READ_TRACKER,
             'rm_action_text',
             pytest.raises(AssertionError)),
        ]
    )
    async def test_delete_action_handler(
        ...
        if actions_lst:
            ...
            if user_id == OTHER_USER_ID:
                data = ActionCD(operation=operation, action_id=14,
                                action_name='my_new_action')
        ...
        with expectation:
            ...
            if not actions_lst:
                ...
                assert handler_returns.reply_markup == await menu_inline_kb(
                    await buttons.actions_btn_source.action_menu_buttons(), i18n)
                assert await is_redis_hexists_tracker(user_id,
                                                      redis_client=redis_cli) is False
