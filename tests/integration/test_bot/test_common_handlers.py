...
class TestCommonHandlers:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (NEW_USER, "/" + CommandName.START.name, 'user_in_db_text',
             does_not_raise()),  # user all ready in db
            (MAIN_USER_ID, "/" + CommandName.START.name, 'user_in_db_text',
             does_not_raise()),  # user all ready in db
            (MAIN_USER_ID, "/" + CommandName.START.name, None, does_not_raise()),
            # checking the answer text when user tracker was launched
            (MAIN_USER_ID, "/" + CommandName.HELP.name, 'new_user_text',
    ...
    async def test_command_start_handler(
            self, create_tracker_fixt_fact, user_id: int, answer_text: str,
            command: str,
            expectation: Any, redis_cli: Redis, buttons: AppButtons,
        ...
        if (user_id == MAIN_USER_ID) and (answer_text is None):
            await create_tracker_fixt_fact(user_id, category_id=1,
                                           category_name='new_cat', action_id='1',
                                           action_name='new_act',
                                           tracker_id='1')
        ...
        with expectation:
            ...
            if await is_redis_hexists_tracker(user_id, redis_cli):
                assert handler_returns.text[:20] == (await started_tracker_info(
    ...
    @pytest.mark.parametrize(
        ...
        [
            (MAIN_USER_ID, AppButtons.general_btn_source.EXIT_BTN.name, None,
             does_not_raise()),
            (MAIN_USER_ID, AppButtons.general_btn_source.EXIT_BTN.name, 'options_text',
             pytest.raises(AssertionError)),
            (MAIN_USER_ID, AppButtons.general_btn_source.CANCEL_BTN.name, None,
             pytest.raises(AssertionError)),
            (12345, AppButtons.general_btn_source.EXIT_BTN.name, 'options_text',
             does_not_raise()),
    ...
    async def test_exit_menu_handler(
            self, execute_callback_query_handler, buttons: AppButtons,
            i18n: TranslatorRunner, redis_cli: Redis, user_id: int, answer_text: str,
            data: str, expectation: Any,
    ) -> None:
        handler_returns: EditMessageText = await execute_callback_query_handler(user_id,
                                                                                data)
        if answer_text is None:
            result_text = await started_tracker_info(user_id=user_id,
                                                     redis_client=redis_cli, i18n=i18n,
    ...
    @pytest.mark.parametrize(
        "user_id, command, answer_text, expectation",
        [
            (MAIN_USER_ID, "/" + CommandName.CANCEL.name, 'canceling_text',
             does_not_raise()),
            (OTHER_USER_ID, "/" + CommandName.CANCEL.name, 'canceling_text',
             does_not_raise()),
            (MAIN_USER_ID, '', 'options_text', pytest.raises(AssertionError)),
            (MAIN_USER_ID, "/" + CommandName.CANCEL.name, 'pass not correct text',
             pytest.raises(AssertionError)),
        ]
    )
    async def test_command_cancel_handler(
            self, execute_message_handler: FactoryFixtureFunction, user_id: int,
            command: str, answer_text: str, expectation: Any, i18n: TranslatorRunner
    ) -> None:
        handler_returns = await execute_message_handler(user_id=user_id,
                                                        text=command)
        with expectation:
    ...
    @pytest.mark.parametrize(
        "user_id, data, answer_text, expectation",
        [
            (MAIN_USER_ID, AppButtons.general_btn_source.CANCEL_BTN.name, 'exit_text',
             does_not_raise()),
            (OTHER_USER_ID, AppButtons.general_btn_source.CANCEL_BTN.name, 'exit_text',
             does_not_raise()),
            (MAIN_USER_ID, None, 'options_text', pytest.raises(AssertionError)),
            (MAIN_USER_ID, AppButtons.general_btn_source.CANCEL_BTN.name,
             'pass not correct text',
             pytest.raises(AssertionError)),
            (MAIN_USER_ID, 'pass not correct data', 'exit_text',
             pytest.raises(AssertionError)),
        ]
    )
    async def test_button_cancel_handler(
            self, execute_callback_query_handler: FactoryFixtureFunction, user_id: int,
            data: str, answer_text: str, expectation: Any, i18n: TranslatorRunner
    ) -> None:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (OTHER_USER_ID, '/help', does_not_raise()),
            (MAIN_USER_ID, '/help_cat', does_not_raise()),
            (OTHER_USER_ID, '/help_act', does_not_raise()),
            (MAIN_USER_ID, '/help_tra', does_not_raise()),
            (OTHER_USER_ID, '/help_rep', does_not_raise()),
            (MAIN_USER_ID, '/help_unknown', does_not_raise()),
            (MAIN_USER_ID, '/help_this_is_incorrect', does_not_raise()),
        ]
    )
    async def test_command_help_handler(
            self, execute_message_handler, user_id: int, text: str,
            expectation: Any, i18n: TranslatorRunner, redis_cli: Redis
    ) -> None:
    ...
    @pytest.mark.parametrize(
        "user_id, command, answer_text, expectation",
        [
            (MAIN_USER_ID, '/' + CommandName.SETTINGS.name, 'options_text',
             does_not_raise()),
            (OTHER_USER_ID, '/' + CommandName.SETTINGS.name, 'options_text',
             does_not_raise()),
            (OTHER_USER_ID, '/' + CommandName.SETTINGS.name, '',
             pytest.raises(AssertionError)),
            (MAIN_USER_ID, 'pass not correct command', 'options_text',
             pytest.raises(AssertionError)),
        ]
    )
    async def test_command_settings_handler(
            self, execute_message_handler, user_id: int, command: str,
            expectation: Any, i18n: TranslatorRunner, answer_text: str,
            buttons: AppButtons
    ) -> None:
        handler_returns = await execute_message_handler(user_id=user_id,
                                                        text=command)
    ...
    @pytest.mark.parametrize(
        "user_id, data, answer_text, expectation",
        [
            (MAIN_USER_ID, AppButtons.settings_btn_source.LANGUAGE.name,
             'select_lang_text', does_not_raise()),
            (OTHER_USER_ID, AppButtons.settings_btn_source.LANGUAGE.name,
             'select_lang_text',
             does_not_raise()),
            ...
            (MAIN_USER_ID, AppButtons.settings_btn_source.LANGUAGE.name,
             'pass not correct text',
             pytest.raises(AssertionError)),
            (MAIN_USER_ID, 'pass not correct data', 'select_lang_text',
             pytest.raises(AssertionError)),
        ]
    )
    async def test_language_settings(
            self, execute_callback_query_handler: FactoryFixtureFunction, user_id: int,
            data: str, answer_text: str, expectation: Any, i18n: TranslatorRunner,
            buttons: AppButtons, redis_cli: Redis
    ) -> None:
        handler_returns = await execute_callback_query_handler(
            user_id=user_id, data=data)
        with expectation:
            assert isinstance(handler_returns, EditMessageText)
            assert handler_returns.text == i18n.get(answer_text)
            local = await redis_hget_lang(user_id, redis_client=redis_cli)
            assert handler_returns.reply_markup == await _get_right_markup(
    ...
    @pytest.mark.parametrize(
        "user_id, data, answer_text, expectation",
        [
            (MAIN_USER_ID, AppButtons.settings_btn_source.X_RUSSIA.name,
             'set_lang_text', does_not_raise()),
            (OTHER_USER_ID, AppButtons.settings_btn_source.RUSSIA.name, 'set_lang_text',
             does_not_raise()),
            (OTHER_USER_ID, AppButtons.settings_btn_source.RUSSIA.name,
             'settings_not_change_text', does_not_raise()),
            (MAIN_USER_ID, None, 'select_lang_text', pytest.raises(AssertionError)),
            (MAIN_USER_ID, AppButtons.settings_btn_source.ENGLISH.name, 'set_lang_text',
             does_not_raise()),
            (OTHER_USER_ID, AppButtons.settings_btn_source.X_ENGLISH.name,
             'set_lang_text', does_not_raise()),
            (MAIN_USER_ID, 'pass not correct data', 'select_lang_text',
             pytest.raises(AssertionError)),
            (MAIN_USER_ID, AppButtons.settings_btn_source.LANGUAGE.name,
    ...
    async def test_set_user_lang(
            self, execute_callback_query_handler: FactoryFixtureFunction, user_id: int,
            data: str, answer_text: str, expectation: Any, i18n: TranslatorRunner,
            redis_cli: Redis, buttons: AppButtons, lang_bot_settings: Any
    ) -> None:
        handler_returns = await execute_callback_query_handler(user_id=user_id,
                                                               data=data)
        name = set_redis_name(user_id, prefix=LANG_PREFIX)
        lang_code_after = await redis_cli.hget(name=name, key=str(user_id))
        with expectation:
